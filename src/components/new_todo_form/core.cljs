(ns components.new-todo-form.core
  (:require [reagent.core :as reagent] 
            ...
            [utils.react :refer-macros [handler-fn]]
...
(defn new-todo-form
  [props & children]
  (let [state (reagent/atom {:title nil :body nil :expanded? false})
        add-todo (fn [data] (dispatch (todo-actions/add data)))]
    (fn [props & children]
      ...
      [:div.new-todo-form
       (when-not (@state :expanded?)
         [primary-button {:class-name "new-todo-button"
                          :full-width true
                          :on-click (handler-fn (swap! state update :expanded? not))}
          "Add new todo"])
       (when (@state :expanded?)
         [mui/paper
